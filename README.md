...
Deploy production ready [authorizer.dev](https://authorizer.dev) instance on [Render](https://render.com/) against **your own existing database** and build with it in 30seconds

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/authorizerdev/authorizer-render/tree/without-postgres)
...
> Note: This branch provisions no database. Set `DATABASE_TYPE` and `DATABASE_URL` to point at one you already run. For a managed Render postgres, deploy the `main` branch instead.
...
| `DATABASE_URL` | Connection string for your existing database **(required)** | `postgresql://user:pass@host:5432/authorizer` |
...
`https://your-app.onrender.com/app,https://yoursite.com/callback` — to get the exact matching
